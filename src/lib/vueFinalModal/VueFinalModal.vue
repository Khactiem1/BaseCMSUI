<template>
	<div
		...
		@keydown.esc="onEsc"
	>
		<transition v-bind="computedOverlayTransition" @before-enter="beforeOverlayEnter" @after-enter="afterOverlayEnter" @before-leave="beforeOverlayLeave" @after-leave="afterOverlayLeave">
			<div v-if="!hideOverlay && visibility.overlay" class="vfm__overlay vfm--overlay vfm--absolute vfm--inset" :class="overlayClass" :style="overlayStyle"></div>
		</transition>
		<transition v-bind="computedTransition" @before-enter="beforeModalEnter" @after-enter="afterModalEnter" @before-leave="beforeModalLeave" @after-leave="afterModalLeave">
			<div
			...
			>
				<div ref="vfmContent" class="vfm__content" :class="[contentClass, { 'vfm--prevent-auto': preventClick }]" :style="bindContentStyle" @mousedown="onMousedown(null)">
					<slot :params="params" :close="() => $emit('update:modelValue', false)" />
					<div v-if="visibility.resize && visibility.modal" ref="vfmResize" class="vfm__resize vfm--absolute vfm--inset vfm--prevent-none vfm--select-none vfm--touch-none">
						<div v-for="direction in resizeDirections" :key="direction" :direction="direction" :class="`vfm--resize-${direction}`" class="vfm--absolute vfm--prevent-auto"></div>
					</div>
				</div>
			</div>
		</transition>
...
import { setStyle, getPosition, capitalize, clamp, trimPx, validDragElement, addListener, removeListener } from './utils/dragResize';
...
export default {
	props: {
		...
		resizeDirections: {
			...
			validator: val => ['t', 'tr', 'r', 'br', 'b', 'bl', 'l', 'tl'].filter(value => val.indexOf(value) !== -1).length === val.length
		},
	...
	setup(props, { emit }) {
		...
		function onEsc() {
			if (visible.value && props.escToClose) {
				//kdlong 28.09.2023 fixbug 39314
				//Nếu có form help thì sẽ không gọi vào đây để ẩn form popup bên ngoài nữa
				let helpForm = document.getElementsByClassName('help-popup');
